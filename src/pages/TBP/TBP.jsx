...
const TBP = () => {
  ...
  const wrapPosition = (position) => {
    return {
      x: ((position.x % dimensions.width) + dimensions.width) % dimensions.width,
      y: ((position.y % dimensions.height) + dimensions.height) % dimensions.height,
    };
  };
  ...
  const draw = (ctx) => {
    ...
    if (dragState.isDragging) {
      ...
      const predictedPath = predictPath(bodies, dragState.bodyIndex, newVelocity);
