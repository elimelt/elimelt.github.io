...
function getWsUrl(channel) {
  return `wss://blink.tail8ab50a.ts.net:8443/ws/chat/${encodeURIComponent(channel)}`;
}
...
function createMessageElement(msg) {
  ...
  if (isPresence) {
    ...
    const time = new Date(msg.timestamp || msg.visitor?.connected_at || Date.now()).toLocaleTimeString();
...
async function fetchHistory(initial = false) {
  ...
  try {
    const res = await fetch(`${BASE_URL}/chat/${encodeURIComponent(state.channel)}/history?${params}`);
...
async function fetchPresenceEvents(before) {
  const params = new URLSearchParams({ topic: 'visitor_updates', limit: '100' });
  ...
  return results.filter(e => !isDuplicate(e)).sort((a, b) =>
...
function handleScroll() {
  const msgsEl = document.getElementById('chat-messages');
  if (!msgsEl) return;
  if (msgsEl.scrollTop < 50 && !state.isLoadingHistory && state.hasMoreHistory) {
    fetchHistory();
  }
}
...
function connectVisitors() {
  ...
  visitorWs.onmessage = (evt) => {
    ...
    if (data.type === 'join' || data.type === 'leave') {
      const event = {
        ...
        timestamp: data.timestamp || data.visitor?.connected_at || new Date().toISOString()
      };
  ...
  visitorWs.onclose = () => setTimeout(connectVisitors, 3000);
}
